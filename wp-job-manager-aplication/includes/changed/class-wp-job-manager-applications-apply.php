...
class WP_Job_Manager_Applications_Apply extends WP_Job_Manager_Form {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			...
			[
				'i18n_required' => __( '"%s" is required to successfully apply for this job.', 'wp-job-manager-applications' ),
			]
		);
	}
			...
			try {
				...
				foreach ( $fields as $key => $field ) {
					...
					if ( $field['required'] && empty( $values[ $key ] ) ) {
						throw new Exception( sprintf( __( '"%s" is required to successfully apply for this job.', 'wp-job-manager-applications' ), $field['label'] ) );
					}
